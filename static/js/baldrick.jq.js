...
bldrk.fn = bldrk.prototype = {
	bindTriggers: function(bindClass){
		jQuery(bindClass+':not(.bldrk)').each(function(){
			this.delay = 0;
			var eventType = (this.nodeName == 'FORM' ? 'submit' : 'click');
		...
		$.fn.doAction = function(options, e){
			...
			this.init = function(ev){
				...
				if(typeof vars.target === 'undefined'){
					...
					target = (jQuery(element.data('target')).length > 0 ? jQuery(element.data('target')) : jQuery('#'+element.data('target')));
				}
				...
				if(request.url === null){
					...
					if(vars.callback){
						if(typeof window[vars.callback] === 'function'){
							clearTarget(element);
							try {
								var cbData = window[vars.callback](e, element);
								request.success(cbData, element, vars.callback);
							}
							catch(e){
								request.error(e, element, vars.callback);
							}
						}
						request.complete();
					}
